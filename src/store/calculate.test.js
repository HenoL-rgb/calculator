...
test('Тест корня', () => {
    ...
    expect(calculate({...testState, values: ['-5','-0.5'], operation: '^'}).value).toBe('Error: neg square');
...
test('Тест деления', () => {
    ...
    expect(calculate({...testState, values: ['-5','0'], operation: '/'}).value).toBe('Error: divide by zero');
...
test('Тест факториала', () => {
    ...
    expect(calculate({...testState, values: ['-2'], operation: 'x!'}).value).toBe('Error: negative factorial');
...
test('Тест 1/x', () => {
    ...
    expect(calculate({...testState, values: ['1','0'], operation: '/'}).value).toBe('Error: divide by zero');
...
test('Тест процента', () => {
    ...
    expect(calculate({...testState, values: ['5', '6'], operation: '%', lastOperation: '+'}).values[0]).toBe('5.3');
    expect(calculate({...testState, values: ['5', '-6'], operation: '%', lastOperation: '*'}).values[0]).toBe('-0.3');
    expect(calculate({...testState, values: ['5', '0.5'], operation: '%', lastOperation: '/'}).values[0]).toBe('1000');
